...
function write_problem_stochastic(problem::StochasticStowageProblem,foldername::String,filename::String)
    # Creates folder for results
    if !isdir("Results/"*foldername)
        mkdir("Results/"*foldername)
    end
    for i in 1:problem.scenarios
        ...
        open(joinpath("Results",foldername,filename*"_scenario_"*string(i)*".json"), "w") do file
            JSON.print(file, cargo, 4)  # Pretty-print with indentation
        end
    end
...
function write_solution(solution::Solution,foldername::String,filename::String)
    if !isdir("Results/"*foldername)
        mkdir("Results/"*foldername)
    end
    ...
    open(joinpath("Results",foldername,filename*"_placements"*".json"), "w") do file
...
function write_solution_stochastic(solution::SolutionStochastic,foldername::String,filename::String)
    if !isdir("Results/"*foldername)
        mkdir("Results/"*foldername)
    end
    placements = [cargo for cargo in solution.cargo]
    ...
    open(joinpath("Results",foldername,filename*"_forces"*".json"), "w") do file
...
function get_stochastic_problem(foldername::String,filename::String,problemname1,problemname2,problemname3)
    open(joinpath("Results",foldername,filename*"_info"*".json"), "r") do file
        ...
        for i in 1:scenarios
            open(joinpath("Results",foldername,filename*"_scenario_"*string(i)*".json"), "r") do file
...
function get_solution_deterministic(foldername::String,filename::String)
    open(joinpath("Results",foldername,filename*"_info"*".json"), "r") do file
        info = JSON3.read(read(file, String), Vector{Any})
        gap = info[1]
        status = info[2]
        ...
        placements = open(joinpath("Results",foldername,filename*"_placements"*".json"), "r") do file
...
function get_solution_stochastic(foldername::String,filename::String)
    open(joinpath("Results",foldername,filename*"_info"*".json"), "r") do file
        info = JSON3.read(read(file, String), Vector{Any})
        gap = info[1]
        status = info[2]
        ...
        forces = open(joinpath("Results",foldername,filename*"_forces"*".json"), "r") do file
