...
# Model where the cargo fraction needed to be loaded is a constraint.
# The stability constraints are slacked, but penalized in the objective function.
function create_model_stochastic_cargo_fraction(problem::StochasticStowageProblem, fraction::Float64)
	...
	# New constraint:
	# determines how much cargo must be loaded
	@constraint(model, sum(cargo_slack[c] for c ∈ 1:n_cargo) >= fraction * n_cargo)
	...
	# Deck weight limit. Constraint (29)
	@variable(model, slack_deck[1:n_deck, 1:scenarios] >= 0) # Slack variable for deck weight limit
	@constraint(model,[d = 1:n_deck, sc = 1:scenarios],
		sum(weight[s,sc] for s in [x.id for x in filter(x -> x.deck_id == d, slots)]) <= vessel.decks[d].weight_limit + slack_deck[d,sc])
	# Rasmus: lcg, vcg, and tcg for cargo. Constraint (30), (31), (32)
	@expression(model, lcg_cargo[sc = 1:scenarios], sum(weight[s,sc] * slots[s].lcg for s ∈ 1:n_slots))
	@expression(model, tcg_cargo[sc = 1:scenarios], sum(weight[s,sc] * slots[s].tcg for s ∈ 1:n_slots))
	# Rasmus: Don't understand why is not using weight[s]
	...
	# Rasmus: y_t is defined in this part of the model
	add_stability_stochastic_slack_all!(vessel::Vessel, model, pos_weight_cargo, lcg_cargo, tcg_cargo, vcg_cargo, scenarios, M)

	# Loading variables from stability model
	ballast_volume = model[:ballast_volume]
	slack_Vmax = model[:slack_Vmax]
	slack_Vmin = model[:slack_Vmin]
	slack_Tmin = model[:slack_Tmin]
	...
	slack_ballast_tanks = model[:slack_ballast_tanks]
