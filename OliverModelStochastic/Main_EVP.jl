...
sc = length(scenarios)
n = length(n_unknown)
Deterministic_Solution[parse_index] = get_solution_deterministic("Finlandia_deterministic",
    "Deterministic_Solution", HPC_folder)
Deterministic_problem[parse_index] = load_data(problemname1, test_instance, problemname3)
for i in 1:repetitions
    for j in 1:sc
        for k in 1:n
            ...
            #   EVP   
            foldername = "EVP_random_sampling_rep$(i)_sc$(sc)_unknown$(n_unknown[1])_time$(time_limit)_slack_$(slack_fraction[j])"
            ...
            Sto_pro_boot = get_stochastic_problem(foldername,
                filename, HPC_folder, problemname1, test_instance, problemname3)
            #   EVP   
            foldername = "EVP_Bootstrap1_rep$(i)_sc$(sc)_unknown$(n_unknown[1])_time$(time_limit)"
            # Save stochastic problems scenarios
            pro_sto = Sto_pro_gen
            pro = expected_value_problem(pro_sto)
            # Save problem
