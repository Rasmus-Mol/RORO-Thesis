...
# Check if stochastic solution is feasible with new weights
function feasibility_check_stochastic(sol::SolutionStochastic, problem::StowageProblem, CargoC::CargoCollection)
    cs = sol.cs
    #tanks = Solution.ballast_volume
    slots = problem.slots
    vessel = problem.vessel
    ...
    for d in 1:n_deck
        ...
        if deck_weight > vessel.decks[d].weight_limit
            ...
            model = second_stage_model_slack(cs, pro)
            set_time_limit_sec(model, 60 * 15) # 15 minutes - should maybe be changed.
            set_silent(model)
            optimize!(model)
            return feasible, str, model
        end
