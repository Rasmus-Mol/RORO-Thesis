...
for i in 1:repetitions
    for j in 1:length(scenario_test)
        ...
        n_cargo_unknownweight[1], scenario_test[j], [], generate_simple_cargo_scenarios, scenario_reduction_heuristic, 5*60)
        pro_naive = create_stochastic_problem_scenarioreduction(problem_det, sc, 
        n_cargo_unknownweight[1], scenario_test[j], [], generate_simple_cargo_scenarios, scenario_reduction_naive, 5*60)
        ...
        foldername_H = "Stochastic_random_sampling_rep$(i)_sc$(sc)_unknown$(n_cargo_unknownweight[1])_time$(time_limit)_Heuristic_sc$(scenario_test[j])"
        write_problem_stochastic(pro_H, foldername_H, "Stochastic_Problem", HPC_folder)
        foldername_naive = "Stochastic_random_sampling_rep$(i)_sc$(sc)_unknown$(n_cargo_unknownweight[1])_time$(time_limit)_Naive_sc$(scenario_test[j])"
        write_problem_stochastic(pro_naive, foldername_naive, "Stochastic_Problem", HPC_folder)
        ...
        # Model - 2
        mo = create_model_stochastic(pro_H)
        set_silent(mo) # removes terminal output
        set_time_limit_sec(mo, time_limit)
        optimize!(mo)
        sol = extract_stochastic_solution(pro_H, mo)
        # Save solution
        write_solution_stochastic(sol, foldername_H, "Stochastic_Solution", HPC_folder)
        cs_sol = sol.cs
        # Solve second stage when we know unknown weights
        second_stage_m = second_stage_model(cs_sol, problem_det)
        set_silent(second_stage_m) # removes terminal output
        ...
        fitted_sol = get_solution_second_stage_stochastic(problem_det, second_stage_m, sol)
        # Save fitted solution
        write_solution(fitted_sol, foldername_H, "Fitted_Solution", HPC_folder)
        # Check if second-stage problem was feasible
        #println(fitted_sol.status)
        if fitted_sol.status == "INFEASIBLE"
            second_stage_m_slacked = second_stage_model_slack(cs_sol, problem_det)
            ...
            optimize!(second_stage_m_slacked)
            fitted_sol_slacked = get_solution_second_stage_stochastic(problem_det, second_stage_m_slacked, sol)
            write_solution(fitted_sol_slacked, foldername_H, "Fitted_Solution_slacked", HPC_folder)
            write_slack(HPC_folder, foldername_H, "Fitted_Solution", second_stage_m_slacked)
        end
        # Model - 3
        mo = create_model_stochastic(pro_naive)
        set_silent(mo) # removes terminal output
        set_time_limit_sec(mo, time_limit)
        optimize!(mo)
        sol = extract_stochastic_solution(pro_naive, mo)
        # Save solution
        write_solution_stochastic(sol, foldername_naive, "Stochastic_Solution", HPC_folder)
        cs_sol = sol.cs
        # Solve second stage when we know unknown weights
        second_stage_m = second_stage_model(cs_sol, problem_det)
        set_silent(second_stage_m) # removes terminal output
        ...
        fitted_sol = get_solution_second_stage_stochastic(problem_det, second_stage_m, sol)
        # Save fitted solution
        write_solution(fitted_sol, foldername_naive, "Fitted_Solution", HPC_folder)
        # Check if second-stage problem was feasible
        #println(fitted_sol.status)
        if fitted_sol.status == "INFEASIBLE"
            second_stage_m_slacked = second_stage_model_slack(cs_sol, problem_det)
            ...
            optimize!(second_stage_m_slacked)
            fitted_sol_slacked = get_solution_second_stage_stochastic(problem_det, second_stage_m_slacked, sol)
            write_solution(fitted_sol_slacked, foldername_naive, "Fitted_Solution_slacked", HPC_folder)
            write_slack(HPC_folder, foldername_naive, "Fitted_Solution", second_stage_m_slacked)
        end
        ...
        write_problem_stochastic(pro, foldername, "Stochastic_Problem", HPC_folder)
        foldername_H = "Stochastic_Bootstrap1_rep$(i)_sc$(sc)_unknown$(n_cargo_unknownweight[1])_time$(time_limit)_Heuristic_sc$(scenario_test[j])"
        write_problem_stochastic(pro_H, foldername_H, "Stochastic_Problem", HPC_folder)
        foldername_naive = "Stochastic_Bootstrap1_rep$(i)_sc$(sc)_unknown$(n_cargo_unknownweight[1])_time$(time_limit)_Naive_sc$(scenario_test[j])"
        write_problem_stochastic(pro_naive, foldername_naive, "Stochastic_Problem", HPC_folder)
